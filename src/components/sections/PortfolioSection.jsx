    ...
    <SectionWrapper id="portfolio" className="w-full overflow-hidden px-4 py-12 md:py-16">
      ...
      <div className="relative w-full">
        <Button 
          ...
          size="icon" 
          onClick={() => scroll('left')} 
          className="absolute left-0 md:-left-4 top-1/2 -translate-y-1/2 transform bg-white/80 hover:bg-white rounded-full shadow-md z-10 border-lumirise-accent text-lumirise-accent hover:text-white hover:bg-lumirise-accent"
        >
        ...
        <div
          ...
          onWheel={handleWheel}
          className={`flex overflow-x-auto pb-4 space-x-4 md:space-x-6 w-full mx-auto snap-x snap-mandatory ${isMobile ? 'h-[320px]' : 'h-auto'}`}
          style={{ 
            scrollbarWidth: 'none',
        ...
        >
          {/* First empty div for centering - mobile specific */}
          <div className={`flex-shrink-0 ${isMobile ? 'w-[20px]' : 'w-[calc((100%-300px)/2)] md:w-[calc((100%-340px)/2)]'}`} />
          
          {portfolioItems.map((item, index) => (
            <motion.div
              key={index}
              className="portfolio-card flex-shrink-0 w-[240px] sm:w-[280px] md:w-[300px] lg:w-[340px] snap-start"
          ...
          {/* Last empty div for centering - mobile specific */}
          <div className={`flex-shrink-0 ${isMobile ? 'w-[20px]' : 'w-[calc((100%-300px)/2)] md:w-[calc((100%-340px)/2)]'}`} />
        </div>
        ...
        <Button 
          ...
          className="absolute right-0 md:-right-4 top-1/2 -translate-y-1/2 transform bg-white/80 hover:bg-white rounded-full shadow-md z-10 border-lumirise-accent text-lumirise-accent hover:text-white hover:bg-lumirise-accent"
        >
